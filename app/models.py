...
def retrieve_trips(username): 
    ...
    with sql.connect('app.db') as con:
        ...
        result = cur.execute("SELECT trip.trip_title, trip.destination FROM trip INNER JOIN trip_user ON trip.trip_id = trip_user.trip_id WHERE trip_user.uid = %s", username).fetchall() 
        print(result)
    return result
